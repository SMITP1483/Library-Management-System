...
public class BorrowBookTest {
    ...
    public void borrowBook_InvalidArgument_ShouldReturnBadRequest() throws Exception {
        ...
        NewUserDetailsDTO newUserDetailsDTO = new NewUserDetailsDTO("", " ", "john@gmail.com");

        // we can also check the validation for the book details argument and date also
        BorrowedRecordDTO borrowedRecordDTO = new BorrowedRecordDTO("978-0-596-52068-7", newUserDetailsDTO, new Date(2024, Calendar.SEPTEMBER, 31, 8, 24));
    ...
    public void borrowBook_BookNotFound_ShouldReturnNotFound() throws Exception {
        NewUserDetailsDTO newUserDetailsDTO = new NewUserDetailsDTO("Jack", "Dason", "jack@gmail.com");

        BorrowedRecordDTO borrowedRecordDTO = new BorrowedRecordDTO("978-0-596-52068-7", newUserDetailsDTO, new Date(2024, Calendar.SEPTEMBER, 31, 8, 24));

        when(borrowBookController.borrowBookWithUserDetails(borrowedRecordDTO)).thenReturn(ResponseEntity.status(HttpStatus.NOT_FOUND).body("Book not found"));

        when(borrowBookController.borrowBookWithUserDetails(any(BorrowedRecordDTO.class))).thenReturn(ResponseEntity.status(HttpStatus.NOT_FOUND).body("Book not found"));
    ...
    public void borrowBook_BookNotAvailable_ShouldReturnNotFound() throws Exception {
        ...
        when(borrowBookController.borrowBookWithUserDetails(borrowedRecordDTO)).thenReturn(ResponseEntity.status(HttpStatus.NOT_FOUND).body("Requested Book is currently Not Available"));
    ...
    public void BookBorrowedByUser_ValidArgument_ShouldReturnOk() throws Exception {
        ...
        BorrowedRecordDTO borrowedRecordDTO = new BorrowedRecordDTO("978-0-596-52068-7", newUserDetailsDTO, borrowedDate);

        when(borrowBookController.borrowBookWithUserDetails(borrowedRecordDTO)).thenReturn(ResponseEntity.status(HttpStatus.OK).body("Book Borrowed"));
